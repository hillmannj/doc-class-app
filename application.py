...
def classify_words(words):
    with application.open(os.path.join(APP_ROOT, 'id_to_label.pkl'),
        'rb') as file:
        id_to_label = joblib.load(file)
    with application.open(os.path.join(APP_ROOT, 'pipeline.pkl'), 'rb') as file:
        pipeline = joblib.load(file)
    with application.open(os.path.join(APP_ROOT,
